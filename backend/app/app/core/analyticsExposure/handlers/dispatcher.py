...
class BaseDispatcher(ABC, Generic[T]):
    ...
    @abstractmethod
    def results_to_analytics_payload(self, handler: AnalyticsHandler[T], results: list) -> T | None:
...
class FetchDispatcher(BaseDispatcher[AnalyticsData]):
    ...
    def results_to_analytics_payload(
        self, handler: AnalyticsHandler[AnalyticsData], results: list
    ) -> AnalyticsData | None:
...
class SubscriptionDispatcher(BaseDispatcher[AnalyticsEventNotif]):
    ...
    @override
    def results_to_analytics_payload(
        self, handler: AnalyticsHandler[AnalyticsEventNotif], results: list
